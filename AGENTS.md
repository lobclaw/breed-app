...
- 记录表单若引入首屏骨架，`edit` 态不能只依赖异步请求后的 `currentRecord.type` / `record.type` 再决定骨架类型；必须在进入 loading 前就拿到稳定类型，或提供可承接的编辑态类型口径，避免编辑页退回成通用骨架
...
- 档案页默认保持稳定业务排序，不因“新增犬只返回后要有感觉”改成全局 `created_at` 倒序；新增后的存在感优先通过来源页承接解决
- 从档案页新增犬只返回列表时，必须消费 `submit feedback` 做弱成功反馈；若新犬只在当前结果中可见，应自动定位并做一次性短暂高亮
- 若新建犬只被当前搜索或筛选挡住，必须保留用户当前上下文；只提示“当前筛选下未显示”，不要自动清空搜索词、重置筛选或改快捷筛选
...
- 记录表单的类型化骨架必须按真实首屏字段块顺序拟真；不要只保留主字段后直接落到通用备注区。图片上传、自动计算卡、额外安排等首屏可见区块若真实存在，骨架也必须保留对应占位
