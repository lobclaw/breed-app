...
- 登录页参考抖音类移动端布局：自定义顶部关闭/返回 + 帮助，正文大标题左对齐，手机号输入整块浅灰底，主 CTA 强粉色，协议在 CTA 下方弱化展示，其他登录方式沉到底部
...
- 改登录 / 家庭成员：核对“登录即自动创建账号”口径、一键登录兜底、验证码/密码入口切换、协议展示、`families.members.nickname` 写入与默认称呼
