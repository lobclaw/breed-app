...
- 短信发送防刷必须使用 `opendb-verify-codes` 的原子发送占位：先创建确定性 `_id` 占位，再检查限流，再发送短信；并发 duplicate 只能按统一错误 `uni-id-sms-send-too-frequent` 处理，不能退回“先查后写”
- 短信占位状态必须区分 `pending / sent / failed / blocked`；限流只统计非 `failed` 占位，发送失败必须标记 `failed` 或释放占位，避免“短信没发出但用户被锁 5 分钟”；`failed` 复用必须用 `_id + status=failed` 条件更新，且只允许抢占一次
- 找回密码 `reset-pwd-by-sms` 发送短信时，未注册手机号按成功返回但不发短信、不写可用验证码，只保留限流占位，避免短信成本与账号枚举；前端统一展示发送成功/倒计时，不暴露账号是否存在
...
- 找回密码验证码使用现有 `uni-captcha` 免费图形验证码，不接阿里云/腾讯云/滑动拼图；短信码在点击“完成”时由 `resetPwdBySms` 校验并重置密码，成功后自动用手机号 + 新密码登录
...
- 改登录 / 家庭成员：核对“登录即自动创建账号”口径、一键登录兜底、验证码/密码入口切换、验证码登录两步流、短信页 6 位固定格、300 秒倒计时、测试码手动输入、找回密码单页流、uni 图形验证码、短信防刷云端原子占位、密码规则、协议展示、`families.members.nickname` 写入与默认称呼
