...
describe('backup page contract', () => {
  it('应在备份前检查本地同步状态并跳转到回收站页', () => {
    ...
    expect(source).toContain('uni.setStorageSync(key, JSON.stringify({')
    expect(source).toContain('localSyncRuntime.getSyncStatus()')
    expect(source).toContain('localSyncRuntime.getOutboxIssues')
    expect(source).toContain('pendingUploadIssues')
    expect(source).toContain('处理附件问题')
    expect(source).toContain('hasPendingUploadIssue')
    expect(source).toContain('isDevMode && syncStatusReady && hasUnsyncedData && syncIssues.length')
    expect(source).toContain('autoSyncPendingAttachments')
    expect(source).toContain('localSyncRuntime.syncPendingAttachmentsNow')
  ...
  it('应区分备份/导出模式并接入真实修复接口', () => {
    ...
    expect(source).toContain('ensureOnline()')
