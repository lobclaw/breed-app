...
describe('password recovery source contract', () => {
  ...
  it('找回密码页应使用单页短信验证码流程、公共图形验证码弹窗，并自动登录', () => {
    ...
    expect(content).toContain("scene: 'reset-pwd-by-sms'")
...
describe('sms login source contract', () => {
  ...
  it('短信验证码发送应走云端统一防刷与统一前端提示', () => {
    const sendSmsCode = source('uniCloud-alipay/cloudfunctions/uni-id-co/module/verify/send-sms-code.js')
    const rateLimit = source('uniCloud-alipay/cloudfunctions/uni-id-co/lib/utils/sms-rate-limit.js')
    const login = source('src/uni_modules/uni-id-pages/pages/login/login-withoutpwd.vue')
    const smsCode = source('src/uni_modules/uni-id-pages/pages/login/login-smscode.vue')
    const retrieve = source('src/uni_modules/uni-id-pages/pages/retrieve/retrieve.vue')
    ...
    expect(rateLimit).toContain('rate_limit_only')
    expect(rateLimit).toContain('RATE_LIMIT_STATUS.FAILED')
    expect(rateLimit).toContain("scene !== SMS_SCENE.RESET_PWD_BY_SMS")
    expect(login).toContain("const SMS_RATE_LIMIT_ERROR = 'uni-id-sms-send-too-frequent'")
    expect(smsCode).toContain("const SMS_RATE_LIMIT_ERROR = 'uni-id-sms-send-too-frequent'")
    expect(retrieve).toContain("const SMS_RATE_LIMIT_ERROR = 'uni-id-sms-send-too-frequent'")
    expect(retrieve).toContain('验证码请求过于频繁，请稍后再试')
  })
})
