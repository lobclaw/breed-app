...
- 配种后直接进入 `怀孕中`
- 生产后进入 `已生产`
- 未配种超时、手动关闭、异常终止记录中的 `放弃配种` 进入 `放弃`
- 未怀孕、流产、死胎、医疗终止进入 `失败`
...
- 日志默认长期保留，不纳入回收站
- 操作日志是云端审计权威，不进入完整 local-first outbox；本地 pending 操作只能作为“待同步操作”合并展示
- 操作日志页可把最近成功拉取的第一页云端日志缓存到本地 storage；缓存只用于离线只读兜底，不作为业务事实源
...
- `定金取消` 是历史终态，犬只去向回到 `在养`；再次销售是犬只维度入口，只挂在同犬最新一条可重启取消记录上，并必须创建新的 `sale_records.status=待售` 记录，旧取消记录不复用、不覆盖
