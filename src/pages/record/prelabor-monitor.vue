...
<template>
  <view class="prelabor-monitor">
    ...
    <!-- 犬只选择卡片 -->
    <view class="prelabor-monitor__dog-card" @click="showDogPicker = true">
      <view class="prelabor-monitor__dog-card-bg" />
      <view class="prelabor-monitor__dog-info">
        <view class="prelabor-monitor__avatar">
          <text class="material-icons-round" style="font-size: 22px; color: #FFFFFF;">pets</text>
        </view>
        <view class="prelabor-monitor__dog-detail">
          <text class="prelabor-monitor__dog-name">{{ selectedDog?.name || '点击选择犬只' }}</text>
          <text class="prelabor-monitor__dog-meta">{{ selectedDog?.meta || '请先选择一只犬' }}</text>
        </view>
      </view>
  ...
  try {
    ...
    if (dog) {
      selectedDog.value = {
        id: dog._id,
        name: dog.name,
        meta: `${dog.breed || '马尔济斯'} · 种母 · ${dog.due_date ? '距预产期' + getDaysUntil(dog.due_date) + '天' : ''}`,
      }
    }
  } catch (e) {
