...
**What:** 首页长期方向固定为「分层首页 + 区块内工作台 + 密度自适应」。保留 `逾期 / 繁育 / 健康 / 用药` 四层结构，但大量任务时不再所有内容都用大卡片直出：
- 逾期：少量展开，多量展示前几条 + 查看更多。
- 繁育：按流程步骤分组，组内轻量犬只/窝行，不做批量完成。
- 健康：按疫苗/驱虫/疾病观察聚合，保留真正适合批量处理的批量卡。
- 用药：按未完成/部分完成/已完成组织，突出今日执行。
- 后续可增加「今日重点」，从各区块抽取最重要的 3-5 件事。
**Why:** 30-50 只犬场景下可能同时出现 20 个繁育计划、20 个健康提醒、10+ 个用药提醒。纯大卡片列表会过长，空间利用率低，用户需要自己从长列表里判断优先级。
**Principles:** 大卡片只用于强行动项或批量操作；大量同类对象使用轻量 row；区块内部按数量自适应展开、折叠、分组。
**Effort:** L（首页信息架构 + 卡片/row 组件 + 服务端分组协议迭代）
